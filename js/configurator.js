(function () {
  ...
  function renderCart() { const packChoice = state.sets.length > 1 ? `<div class="pack-choice"><label class="pack-option ${state.cartMode === 'loose' ? 'selected' : ''}"><input type="radio" name="cartMode" value="loose" ${state.cartMode === 'loose' ? 'checked' : ''}> <strong>Losse sets</strong><small>Elke set apart verpakt</small></label><label class="pack-option ${state.cartMode === 'combined' ? 'selected' : ''}"><input type="radio" name="cartMode" value="combined" ${state.cartMode === 'combined' ? 'checked' : ''}> <strong>Eén gezamenlijke omdoos</strong><small>Jou sets per combinatie in een omdoos</small></label></div>` : ''; return `<div class="journey-kicker">Winkelmand</div><h2>Je aanvraag staat klaar.</h2><p class="lead">${state.sets.length} ${state.sets.length === 1 ? 'set is' : 'sets zijn'} toegevoegd, en staan ook al in je <a href="../cart/index.html">winkelmand</a>.${state.sets.length > 1 ? ' Kies hoe we ze voor je verpakken.' : ''}</p><div class="cart-list">${state.sets.map((set, index) => { const setProducts = normalizeProductSelection(set.products || []); const productText = setProducts.length ? ` · ${setProducts.map((productIndex) => catalog[productIndex]?.name).filter(Boolean).join(', ')}` : ''; return `<div class="cart-item"><span class="cart-number">0${index + 1}</span><div><strong>Eigen service-set</strong><small>${set.quantity.toLocaleString('nl-NL')} sets · Case ${set.branch}${productText}</small></div><button aria-label="Set verwijderen" data-action="remove-${index}">×</button></div>`; }).join('')}</div>${packChoice}<div class="journey-actions">${button('Nog een set samenstellen', 'new-set', 'journey-btn-quiet')}${button('Naar aanvraag', 'checkout', 'journey-btn-primary')}</div><div class="journey-actions" style="margin-top:8px;"><a class="link-arrow" href="../cart/index.html">Bekijk winkelmand →</a></div>`; }
  ...
  // Mirrors a finished custom set into the shared shop cart (js/cart.js) so it
  // shows up in the real winkelmand/checkout alongside regular shop products.
  // Custom sets have no fixed price yet, so they're added as quote items.
  function addConfiguredSetToCart(current, quantity, branch) {
    const setId = 'custom-set-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7);
    if (!window.CartStore) return setId;
    const products = normalizeProductSelection(current.products || []).map((index) => catalog[index]?.name).filter(Boolean);
    const additions = (current.ownArticles || []).map((index) => recommendedAdditions[index]?.name).filter(Boolean);
    const contentSummary = [...products, ...additions].join(', ') || 'Geen inhoud gekozen';
    window.CartStore.addItem({
      id: setId,
      name: `Eigen samenstelling · ${quantity.toLocaleString('nl-NL')} sets`,
      cat: `Case ${branch} · ${contentSummary}`,
      price: null
    }, 1);
    if (window.showCartToast) window.showCartToast('Eigen samenstelling toegevoegd aan winkelmand');
    return setId;
  }
  ...
  function handle(action) {
    ...
    else if (action === 'add-set') { const cartId = addConfiguredSetToCart(state.current, state.quantity, state.branch); state.sets.push({ ...state.current, quantity: state.quantity, branch: state.branch, cartId }); state.step = 'cart'; }
    ...
    else if (action.startsWith('remove-')) { const removed = state.sets.splice(Number(action.split('-')[1]), 1)[0]; if (removed && removed.cartId && window.CartStore) window.CartStore.removeItem(removed.cartId); if (state.sets.length <= 1) state.cartMode = 'loose'; }
    else return;
    render();
  }
